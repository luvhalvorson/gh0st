...
DWORD WINAPI CClientSocket::WorkThread(LPVOID lparam)   
{
	...
	while (pThis->IsRunning()) // 主控端沒關掉就一直在這個loop
	{
		fd_set fdRead = fdSocket;
		int nRet = select(NULL, &fdRead, NULL, NULL, NULL); // 判斷是不是斷開連結
		...
		{
			memset(buff, 0, sizeof(buff));
			int nSize = recv(pThis->m_Socket, buff, sizeof(buff), 0);// 接收主控端傳來的data
			
			if (nSize <= 0)
			{
				pThis->Disconnect(); // 會有一些接收錯誤的處理
				break;
			}
			if (nSize > 0) pThis->OnRead((LPBYTE)buff, nSize); //接收沒問題就call OnRead()
		}
	}
