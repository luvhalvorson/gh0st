...
void CIOCPServer::OnAccept()
{
   ...
   // Associate the new socket with a completion port.
	...
	// ¹Ø±ÕnagleËã·¨,ÒÔÃâÓ°ÏìÐÔÄÜ£¬ÒòÎª¿ØÖÆÊ±¿ØÖÆ¶ËÒª·¢ËÍºÜ¶àÊý¾ÝÁ¿ºÜÐ¡µÄÊý¾Ý°ü,ÒªÇóÂíÉÏ·¢ËÍ
	// ÔÝ²»¹Ø±Õ£¬ÊµÑéµÃÖªÄÜÍøÂçÕûÌåÐÔÄÜÓÐºÜ´óÓ°Ïì
	const char chOpt = 1;
...
void CIOCPServer::Send(ClientContext* pContext, LPBYTE lpData, UINT nSize)
{
	...
	try
	{
		if (nSize > 0)
		{
			...
			pContext->m_ResendWriteBuffer.Write(lpResendWriteBuffer, nSize);	// ±¸·Ý·¢ËÍµÄÊý¾Ý
		...
		else // ÒªÇóÖØ·¢
		{
			pContext->m_WriteBuffer.Write(m_bPacketFlag, sizeof(m_bPacketFlag));
			pContext->m_ResendWriteBuffer.ClearBuffer();
			pContext->m_ResendWriteBuffer.Write(m_bPacketFlag, sizeof(m_bPacketFlag));	// ±¸·Ý·¢ËÍµÄÊý¾Ý	
		}
...
bool CIOCPServer::OnClientReading(ClientContext* pContext, DWORD dwIoSize)
{
	...
	}catch(...)
	{
		...
		// ÒªÇóÖØ·¢£¬¾Í·¢ËÍ0, ÄÚºË×Ô¶¯Ìí¼ÓÊý°ü±êÖ¾
